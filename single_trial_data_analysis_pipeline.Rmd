...
For the example, first a `TD` object is created for experiments conducted in 2012 - 2013.

```{r createTD}
## Create a TD object containing data for 2012 & 2013.
dropsTD <- createTD(data = dropsRaw[dropsRaw$year %in% c(2012, 2013), ],
                    genotype = "Variety_ID", 
                    trial = "Experiment",
                    loc = "Site",
                    repId = "Replicate", 
...
options("statgen.genoColors" = c("blue", "green", "#e8e512"))
...
plot(dropsTD, 
     plotType = "cor", 
     traits = "grain.yield") # plot other traits
```
...
# change colname of BLUP values table
colnames(predDrops$Gai12W$BLUPs) <- c("genotype", "BLUP_grain.yield")

# change colname of BLUE values table
colnames(predDrops$Gai12W$BLUEs) <- c("genotype", "BLUE_grain.yield")

# put together in a single dataframe
predDrops$Gai12W$BLUPs %>% 
  left_join(predDrops$Gai12W$BLUEs, by = "genotype") %>% 
  write.csv("BLUP_BLUE.csv", row.names = F)
...
BLUEsDrops2 <- extractSTA(STA = modDropsSp, 
                          ...
                          keep = "scenarioWater")
# Donwload the table
write.csv(BLUEsDrops2, "BLUEsDrops2.csv", row.names = F)
...
# convert the list into a dataframe and save on my computer
do.call(rbind, TDGxE2) %>% 
  write.csv("GxE_data.csv", row.names = F)
